...
const swalInicio = () =>{
    Swal.fire(
        '¿Jugamos?',
        'Gana el primero en obtener 3 puntos',
        'question'
    )
}
swalInicio();
...
const SwalGanador = () =>{
    Swal.fire({
        icon: 'success',
        tittle: 'Ganas 1 punto',
        text: 'Gantaste, excelente trabajo',
...
const SwalPerdedor = () =>{
    Swal.fire({
      icon: 'error',
        tittle: 'Perdiste, intentalo de nuevo',
        text: '¿Te gana una maquina? Turbio',
        confirmButtonText: '¿De nuevo?',
...
const resultado = () => {
    if (eleccionUsuario == "piedra") {
        ...
        if (eleccionPc == "papel") {
            labelResultado.textContent = "¡Perdiste! ☠️☠️";
            contadorPc++;
            puntajePC.textContent = contadorPc;
